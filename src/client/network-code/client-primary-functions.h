 ...
 * Currently, only the user-facing wxWidgets GUI client driver program sets it.
 */
void (*force_user_out_of_room)(void);

/* Required client-side initialization before we construct message_00 to begin
...
u8 self_init(u8* password, int password_len, const char* save_dir)
{
    ...
    /* Fill in the first set of parameters to Argon2. */
    ...
    /* Set the rest of the parameters to Argon2. */
    ...
    /* Carry out the Argon2 algorithm now. This gets us a 32-byte hash that we
     * use as a cryptographic key in ChaCha20 to decrypt the private key.
     */
    Argon2_MAIN(&prms, argon2_output_tag);

    /* Let V be the leftmost 32 (chacha_key_len) bytes of Argon2's output hash.
     * Use V as the cryptographic key in ChaCha20, along with the saved 16-byte
     * Nonce (from user's save file) to decrypt the user's saved private key.
     */
    memcpy(V, argon2_output_tag, chacha_key_len);

    /* Decrypt the user's saved private key. */
    ...
    /* Compute a public key with that private key and the self-discovered
     * Diffie-Hellman constants M, Q, G. If it's the same as the public key
     * stored on the filesystem, the private key was decrypted successfully,
     * with the user's original correct password.
     */
    ...
    /* Prime order Q, exactly dividing (M - 1). */
    ...
    /* Initialize the Diffie-Hellman shared secret with the server. */
    ...
    /* Extract the bidirectional pair of session keys (KBA, KAB). */
    /*  On client's side:
...
void* begin_polling(__attribute__((unused)) void* input)
{
    ...
    for(;;)
    {
        ...
        else if(*reply_type_ptr == PACKET_ID_20)
        {
             ...
             * for the messages sub-window and the "exit room" button. GUI code
             * will do that if it sees a 0 returned from here.
             */
            flag_no_poll_reply = 1;
        }
...
*/
        if(*reply_type_ptr == PACKET_ID_41)
        {
            ...
            /* Start at first message's contents. */
            ...
            for(u64 i = 0; i < pending_messages; ++i)
            {
                ...
                else if(curr_msg_type == PACKET_ID_51)
                {
                     ...
                     * alert the GUI subsystem to remove GUI elements for
                     * sending messages and exiting the chat room, since it
                     * has just been closed by the room owner.
                     */
                    #ifdef USE_WX_GUI
                ...
                else if(curr_msg_type == PACKET_ID_30)
                {
                    ...
                    /* Displaying a received message. If not using the wxWidgets
                     * user-facing GUI client driver, then the messages are
                     * written to a regular linux terminal window. Else, use
                     * the special function pointer that will have been set to
                     * the appropriate GUI function for rendering messages.
                     */
                    #ifndef USE_WX_GUI
 ...
 * gracefully unblocked from the TUI scanf() that reads user input messages.
 *
 * The user-facing wxWidgets GUI client driver, on the other hand, uses a
 * function pointer and conditional compilation to detect that the GUI is being
 * used and hence, said function pointer has been set to a GUI event handler
 * function for the event that the room we are in has been closed by the owner.
 * In that case, this signal handler is not used at all.
 */
#ifndef USE_WX_GUI
...
/* Start the thread function that sends poll packets to the Rosetta server
 * in an infinite loop and processes the several possible answer packets.
 */
...
/* The user has requested to be registered in the Rosetta system. This produces
 * their local user save file which keeps the user's encrypted private key that
 * can only be unlocked using a key generated by Argon2 upon entering a correct
 * password, among other cryptographic artifacts.
 */
u8 reg(u8* password, int password_len, const char* save_dir)
{
    ...
    /* Generate the user's long term public key A, from the private key. */
    A_longterm = gen_pub_key(&temp_privkey);

    /* Registration step 2: Use the password as a secret key in Argon2, whose
     *                      output hash we use as a cryptographic key in
    ...
    /* Call Blake2b to get component 2 of the Argon2 Salt parameter. */
    ...
    /* Carry out the Argon2 algorithm. Its produced hash is used as the key in
     * ChaCha20 to encrypt/decrypt the user's stored private key.
     */
    Argon2_MAIN(&prms, argon2_output_tag);

    /* Let V be the leftmost 32 bytes of Argon2's hash. Use V as a key in
     * ChaCha20, along with a randomly-generated 16-byte Nonce that will be
     * saved, to encrypt the user's private key.
     */
    ...
    /* Carry out chacha20 to encrypt the user's private key. */
     ...
     *                      ChaCha20 Nonce, Argon2 Salt string, long-term
...
/* The user has decided to get logged in to Rosetta and establish a connection
...
/* The user has decided to create a new chat room. Ask the server to facilitate
 * this action for the client.
 */
...
/* The user has decided to join an existing chat room. */
...
/* The client has decided to send a message to everyone in their chat room. */
...
/* The user has decided to leave the chat room they are in. */
...
/* The user has decided to log out and exit Rosetta. */
u8 logout(void)
{
    u64 msg_len;
    u8  status = 0;
