/* This macro is used in the high-level C client communications component
 * to distinguish between the client having been started with this GUI system
 * and having been started with the Rosetta Test Framework, which would not
 * have a GUI to display messages on, rather displaying them on a TUI.
 */
#define USE_WX_GUI
 ...
 *
 * Client polled the server and received a new message from somebody in their
 * chatroom. Display it on the user's chat room GUI window.
 */
void display_gui_message(char* message_line)
{
    wxString msg_to_display = wxString::FromUTF8(message_line);
    g_instance->GetEventHandler()->CallAfter([msg_to_display]()
    {
        g_instance->msg_entries->AppendText(msg_to_display);
 ...
 * been closed by the room owner. Inform the user on their GUI window that they
 * have been booted from the room as it's been closed for everybody.
 */
void display_gui_user_booted()
{
    g_instance->GetEventHandler()->CallAfter([]()
    {
        ...
        g_instance->info_msg_box->WriteText
          ("Chat room has been closed by the room owner.");
                  ...
                  ,wxSize(1920, 1080)) /* Size of the window in pixels       */
{
     ...
     *            handlers here to display messages on the user's GUI window.
     */
    g_instance = this;
    ...
    /* Create a Monospace Bold and Monospace Regular font. */
    ...
    /* Initialize the GUI login button and the rest of the buttons. */
    ...
    /* Initialize GUI boxes for text input and displaying. */
    ...
    usermsg_input = new wxTextCtrl
      (this, 10017, "", wxPoint(525, 850), wxSize(800, 50), wxTE_PROCESS_ENTER);
    ...
    info_msg_box = new wxTextCtrl
                           (this, wxID_ANY, "", wxPoint(725, 880),
                            wxSize(600, 120), wxTE_READONLY | wxTE_MULTILINE);
    ...
    msg_entries = new wxTextCtrl
                          (this, wxID_ANY, "", wxPoint(525, 440),
                           wxSize(800, 400), wxTE_READONLY | wxTE_MULTILINE);
    ...
    ROSETTA_LABEL = new wxTextCtrl
        (this, wxID_ANY, LOGO_STRING, wxPoint(150, 10), wxSize(1600, 400),
...
void cMain::BtnClickLoginGo(wxCommandEvent &evt)
{
    ...
    strncpy((char*)password ,(const char*)pwd_as_wxstring.mb_str(wxConvUTF8),
            password_len);
...
void cMain::BtnClickRegGo(wxCommandEvent &evt)
{
    ...
    /* Display error box that something went wrong and to try again. */
    if(register_status)
    {
        info_msg_box->SetValue("");
    ...
    /* Update GUI to reflect the successful registration. */
    else
    {
        info_msg_box->SetValue("");
...
void cMain::BtnClickJoinRoomGo(wxCommandEvent &evt)
{
    ...
    strncpy((char*)roomid, (const char*)roomid_as_wxstring.mb_str(wxConvUTF8)
            ,roomid_len);

    memset(userid, 0x00, SMALL_FIELD_LEN);

    strncpy((char*)userid, (const char*)userid_as_wxstring.mb_str(wxConvUTF8),
            userid_len);

    joinroom_status = join_chatroom
                       (roomid, roomid_len, (unsigned char*)userid, userid_len);
...
void cMain::BtnClickMakeRoomGo(wxCommandEvent &evt)
{
    ...
    makeroom_status = make_new_chatroom
                       (roomid, roomid_len, (unsigned char*)userid, userid_len);
...
void cMain::BtnClickSendMsg(wxCommandEvent &evt)
{
    ...
    {
        std::cout << "Entered ret on error. msg_len: " << msg_len << "\n";
        usermsg_input->SetFocus();
        evt.Skip();
        return;
    }
    strncpy((char*)msg_buf, (const char*)(msg_as_wxstring.mb_str(wxConvUTF8)),
            msg_len);

    /* Transmit the user's message to the Rosetta Server. It will authenticate
     * this client and relay the message to everyone in their chat room.
     */
    ...
    /* Display the user's entered message into their own GUI window too. */
