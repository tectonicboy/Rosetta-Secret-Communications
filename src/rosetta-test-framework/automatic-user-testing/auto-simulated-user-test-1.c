...
int main(){
    ...
    printf("[OK]  RTF: In simulation 1. Reset STABILIZED_AVERAGES.dat\n");

    FILE* stabilized_averages_dat_fd = fopen
        ("/home/hypervisor123/tmp/repos/Rosetta-Secure-Texting/"
         "performance-analysis/latest-stabilized-averages.dat", "w");

    if(stabilized_averages_dat_fd == NULL)
    {
        perror("[ERR] RTF: Simulation 1: STABILIZED_AVERAGES open fail: ");
        exit(1);
    }
...
label_again:
    ...
    printf("\nSIMULATION 1 --> MEASURING: Final reduction mod M, manual\n");

    system
     ("PYTHONPATH=/home/hypervisor123/.local/lib/python3.14/site-packages"
      "python3 /home/hypervisor123/tmp/repos/Rosetta-Secure-Texting/"
               "performance-analysis/process-measurements.py");
